...
TcpStreamClientHelper::TcpStreamClientHelper (Address address, uint16_t port , const Ptr<PhyRxStatsCalculator> crossLayerInfo)
{
  m_factory.SetTypeId (TcpStreamClient::GetTypeId ());
  SetAttribute ("RemoteAddress", AddressValue (address));
  SetAttribute ("RemotePort", UintegerValue (port));
...
TcpStreamClientHelper::InstallPriv (Ptr<Node> node, std::string algo, uint16_t clientId) const
{
  ...
  app->GetObject<TcpStreamClient> ()->Initialise (algo, clientId, m_crossLayerInfo);
