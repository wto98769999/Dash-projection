...
void TcpStreamServerHelper::SetAttribute(std::string name,
...
ApplicationContainer TcpStreamServerHelper::Install(
...
TcpStreamClientHelper::TcpStreamClientHelper(Ipv6Address address,
                                             uint16_t port) {
  m_factory.SetTypeId(TcpStreamClient::GetTypeId());
  SetAttribute("RemoteAddress", AddressValue(Address(address)));
  SetAttribute("RemotePort", UintegerValue(port));
...
void TcpStreamClientHelper::SetAttribute(std::string name,
                                         const AttributeValue &value) {
  m_factory.Set(name, value);
}
...
Ptr<Application> TcpStreamClientHelper::InstallPriv(Ptr<Node> node,
                                                    std::string algo,
