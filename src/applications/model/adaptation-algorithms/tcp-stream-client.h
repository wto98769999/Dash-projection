...
public:
   ...
   *
   * A new adaptation algorithm object is created, by calling a constructor of
   * an existing subclass of AdaptationAlgorithm.
   *
...
private:
  ...
  /**
   * \brief This enum is used to define the controller events of the state
   * machine which controls the behaviour of the client.
   */
 ...
 *
 * When a client object is created, it is initiated as follows and finds
 itself in state initial:
 ...
 * - The reduced version of the MPD, containing the duration of a segment in
 microseconds and a (n x m) matrix consisting of n representations and m
 segment sizes, denoted in bytes, is being read in from the file specified at
 program start.
 ...
 * After these initialisations, which take place at object creation, a TCP
 ...
 connection and for receiving are set. Then, the controller does the
 transition initial init-> downloading by calling RequestRepIndex (), thus
 obtaining the next representation level to be downloaded. The client then
 requests the determined segment size from the server by sending it a string
 composed of the number of bytes of the segment. After the request is
 processed by the server, it starts sending the first TCP packet to the
 client. The receiving of a packet notifies the socket that new data is
 available to be read, so the aforementioned SetRcvCallback is triggered and
 the client stars receiving packets. Meanwhile all arrived packets are being
 logged. This is repeated until the received amount of data matches the
 requested segment size. Then, the throughput is logged and the receive
 function calls the controller with the event  downloadFinished.
 * The controller then adds a segment to the buffer and calls the
 ...
 segment, thus simulating the beginning of playback. Then, the function
 returns to the controller, where a timer of m_segmentDuration microseconds is
 set to call PlaybackHandle() again, after playback of the prior segment is
 finished. Next, the requests the next segment as described before. Therefore,
 the controller does the transition downloading downloadfinished->
 downloadingPlaying.
 ...
 * - downloadingPlaying downloadFinished-> playing: download of a segment is
 ...
 only.
 * - downloadingPlaying playbackFinished-> downloading: playback of a segment
 is finished. This event is triggered by the beforehand set timer. The
 controller calls PlaybackHandle(); if the number of segments in the buffer is
 == 0, a buffer underrun is logged.
 * - downloadingPlaying downloadFinished-> playing: download of the last
 segment is finished. Playback of the remaining segment(s) in the buffer
 continues. After finishing playback of all remaining segments in the buffer,
 playing playbackFinished-> terminal is performed, thus closing the client's
 socket, the streaming session for this client ends.
 * Assuming that a buffer underrun has just been encountered and the client is
 ...
 downloadFinished-> playing is performed, the last segment is played and
 playing playbackFinished-> terminal is performed, as explained before.
 */
  void Controller(controllerEvent action);
  /**
   * Set the data fill of the packet (what is actually sent as data to the
   * server with m_data) to the zero-terminated contents of the T & message
   * string.
   *
   ...
   *
   * Before a packet is sent, PreparePacket( T & message) is called to fill the
   ...
   * It increments m_bytesReceived by the number of bytes received and calls
   * SegmentReceivedHandle() when m_bytesReceived == size of segment that is
  ...
  /**
   * \brief triggered by SetConnectCallback if a connection to a host was
   * established.
   */
  void ConnectionSucceeded(Ptr<Socket> socket);
  /**
   * \brief triggered by SetConnectCallback if a connection to a host could not
   * be established.
   */
  void ConnectionFailed(Ptr<Socket> socket);
  /**
   * Called after a segment was completely received from the server, meaning
   * that the received number of bytes == the requested number of bytes.
   * Throughput data and buffer data is logged.
   */
  void SegmentReceivedHandle();
  /**
   * \brief Read in bitrate values
   *
   * The test bitrate values to be read must be provided in bytes in absolute
   * sizes (not per second!) as a 2x2 matrix, with spaces separating the segment
   * sizes and newlines for every representation level.
   */
  int ReadInBitrateValues();
  /*
   * \brief Controls / simulates playback process
   *
   * Gets called by a timer, when the simulated playback of a segment is
   * finished. If m_segmentsInBuffer > 0, then m_segmentsInBuffer is decremented
   * and m_currentPlaybackIndex is incremented. Also, if there was a buffer
   * underrun before, m_bufferUnderrun is set to false and the end of a buffer
   * underrun is logged. If m_segmentsInBuffer == 0, a buffer underrun is
   * registered by writing the event in the bufferUnderrun logfile and
   * m_bufferUnderrun is set to true.
   *
   ...
   *
   * The client requests the next representation index by calling
   * algo->GetNextRep (int64_t m_segmentCounter), which is the interface between
   * an adaptation algorithm and a client, specifying the current segment index
   * as an argument. The algorithm returns an algorithmReply struct, the
   * received values are stored in local variables for logging purposes.
   */
   ...
   * - point in time when transmission started, i.e. arrival of first packet of
   * the whole segment
   * - point in time when transmission ended, i.e. arrival of last packet of the
   * whole segment
   ...
   *
   * Logging the arrival time of fully downloaded segment and logging the buffer
   * level by adding m_segmentDuration on the buffer level.
   *
   ...
   * - the point in time when the decision in the algorithm was made which
   * representation to download next
   * - the case in which the decision was made which representation to download
   * next
   * - the case in which the decision was made if the next download should be
   * delayed \param answer containing the answer the adaptation algorithm has
   * provided.
   */
   ...
   * and log files containing the used adaptation algorithm are created for
   * output.
   */
  ...
  int64_t m_currentRepIndex;  //!< The index of the currently requested segment
                              //!< quality
  int64_t m_lastSegmentIndex; //!< The index of the last segment, i.e. the total
