...
void TcpStreamClient::Initialise(std::string algorithm, uint16_t clientId) {
  ...
  if (algorithm == "tobasco") {
    ...
    bandwidthAlgo = new BandwidthHarmonicAlgorithm(m_videoData, m_playbackData,
                                                   m_bufferData, m_throughput);
    bufferAlgo = new bufferAdaptiveAlgorithm(m_videoData, m_playbackData,
                                             m_bufferData, m_throughput);
    ...
    bufferAlgo = new BufferCleanAlgorithm(m_videoData, m_playbackData,
                                          m_bufferData, m_throughput);
    algo = new Sara1Algorithm(m_videoData, m_playbackData, m_bufferData,
                              m_throughput);
...
void TcpStreamClient::RequestRepIndex() {
  ...
  if (m_algoName == "tobasco") {
    ...
    answer = algo->GetNextRep(m_segmentCounter, m_clientId,
                              bandwidthanswer.bandwidthEstimate, 0);
  } else if (m_algoName == "constbitrateH" || m_algoName == "constbitrateW" ||
             m_algoName == "constbitrateT" || m_algoName == "constbitrateC" ||
             m_algoName == "constbitrateL" || m_algoName == "constbitrateWH") {
    userinfoanswer =
        userinfoAlgo->UserinfoAlgo(m_segmentCounter, m_clientId, 0, 0);
    bandwidthanswer =
        bandwidthAlgo->BandwidthAlgo(m_segmentCounter, m_clientId, 0, 0);
    bufferanswer =
        bufferAlgo->BufferAlgo(m_segmentCounter, m_clientId, 0, 5000000);
    int64_t constRepIndex = 3;
    answer = algo->GetNextRep(m_segmentCounter, m_clientId,
                              bandwidthanswer.bandwidthEstimate, constRepIndex);
  } else {
    NS_LOG_ERROR("Invalid algorithm name entered. Terminating.");
    StopApplication();
    Simulator::Stop();
  ...
  NS_LOG_DEBUG("**At :" << std::fixed << std::setprecision(3)
                        << answer.decisionTime / 1000000.0 << ", REP "
                        << m_segmentCounter << ", BRT "
                        << (m_currentRepIndex + 1) * 10 << ", EBW "
                        << std::fixed << std::setprecision(3)
                        << answer.estimateTh / 1000000.0 << ", DET "
                        << std::fixed << std::setprecision(3)
...
void TcpStreamClient::LogDownload() {
  ...
  downloadLog << std::setfill(' ') << std::setw(5) << m_segmentCounter
              ...
              << std::setw(5) << m_videoData.userInfo.at(m_segmentCounter)
              << std::setfill(' ') << std::setw(9) << instantBitrate << "\n";
  downloadLog.flush();
}
